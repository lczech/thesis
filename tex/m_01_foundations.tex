...
perform a plethora of different functions within organisms.
They are assembled in a process called protein (bio)synthesis,
which is described by the central dogma of molecular biology \cite{Crick1958,Crick1970}:
...
% Thus, the amino acid sequence can also be seen as information storage.
...
It is remarkable that these basic principles and mechanisms -- DNA, amino acids, and the genetic code --
are virtually identical for all living organisms.
This strongly suggests that all life on this planet is related and descends from a common ancestor.

It further implies that by understanding the genetic information
...
put this and the following in section ``genome analysis'' or ``sequence analysis''?

use section ``molecular biology'' for all of this? not really fitting, as we do not do 3d struct, proteins, pathsways etc...

introduce ``genome''

character representing acgt

Because of this pairing,
the length of a DNA sequence is measured in \emph{base pairs} (abbreviated \si{\basepair}):
\SI{1}{\basepair} represents one character of either \nba, \nbc, \nbg, or \nbt along one strand of the DNA.
The character on the opposing strand is then the respective
...
introduce ssu, 16s, 18s

metagenomics, metagenetics, env studies, sampling etc
samples from locations, points in time, etc \todo{find passage in paper/or wherever it is copied here -- found it: it is later in this file!}
introduce ``sample''
...
introduce ``indels'' and homologous chars
