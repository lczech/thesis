...
which classifies organisms in a hierarchy based on common decent.
...
to be useful in understanding many evolutionary relationships.
...
These taxa are associated with a \emph{taxonomic rank}, which can subsume other ranks,
...
While early classifications used phenotypes, that is, observable characteristics or traits of organisms,
modern approaches to taxonomy take genetic information into account.
...
there might be incongruence.

then, classification refined by dna data, resolve ``multifurcations'' and get a strictly bifurcating branching pattern,
where a common ancestor splits into two lineages
...
see \figref{fig:tree_of_life} for examples of phylogenetic trees.

wiki:
The exact relationships of the three domains are still being debated, as is the position of the root of the tree.
% https://en.wikipedia.org/wiki/File:Phylogenetic_tree.svg

tree tips represent differnt things:

from alexey:
Please note, that tips of a phylogenetic tree can represent species, strains, individuals, or even
cells of a multicellular organism. Throughout this thesis, we will use an generic term taxon (plural:
taxa) to refer to all of the above.

but also consensus sequences.
in the end, a tree can be calcualted from any set of sequences.
...
visualization and drawing options: circular, rectangular etc

introduce ``monophyletic'': common ancestor, and all part of the group.
paraphyletic
%  see https://en.wikipedia.org/wiki/Evolutionary_taxonomy
