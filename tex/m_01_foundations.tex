...
This is due to efficiency variations in the sequencing process or biases introduced by the amplification process,
...
see for instance \secref{ch:Visualization:sec:EdgeCorrelation}.
...
As the two samples being compared need to have equal masses in order to enable the transformation,
the KR distance operates on normalized samples; that is, it compares relative abundances.
...
and results in a pairwise distance matrix between the samples.
...
This allows a further generalization by introducing an additional parameter $p$,
