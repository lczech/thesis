...
That is, we do not cover the tree search itself here, but describe how to compute the likelihood $\mathcal{L}$
...
for example $N = \left\{~ \nucleobase{A}, \nucleobase{C}, \nucleobase{G}, \nucleobase{T} ~\right\}$.
Furthermore, for simplicity, we do not consider rate heterogeneity among sites here.
...
This can be thought of as moving along the branches towards the parent nodes,
which induces mutations in form of state transitions.
...
and the branch lengths $b$ and the $P$-matrices of the branches towards the parent are needed.
