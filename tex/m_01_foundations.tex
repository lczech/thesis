...
most often barcoding regions or marker genes are used (see \secref{ch:Foundations:sec:SequenceAnalysis:sub:Metagenomics}).
...
We however here propose an automated approach for assembling suitable sets of reference sequences,
see \secref{ch:AutomaticTrees}.
\todo{this should be Chapter instead of Section...}
Distinct samples from one study are typically placed on the same underlying \ac{RT}
in order to facilitate comparisons between the samples, see \secref{ch:Foundations:sec:PhylogeneticPlacementAnalysis}.
...
which are however omitted in the following, for simplicity of notation.
...
\section{Phylogenetic Placement Analysis}
\label{ch:Foundations:sec:PhylogeneticPlacementAnalysis}
...
% \label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub::par:}

there is also a recent approach that yields comparable results (also jplace),
but uses an alignment-free, non ML approach \cite{Linard2018}.
rappas (aln free)
...
\paragraph{Use Cases and Downstream}
% \label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub::par:}

The information of phylo placement represents useful biological knowledge \emph{per se}.
...
\label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub:Normalization}
...
\label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub:EdgeMasses}
...
\label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub:EdgeImbalances}
...
\label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub:Distances}
...
\label{ch:Foundations:sec:PhylogeneticPlacementAnalysis:sub:ExistingMethods}
