...
% However, it introduces many other terms alongside, which might be used later on.
...
The used marker genes should be present in (most of) the organisms of interest,
...
the 16S and 18S rDNA regions are used as marker genes, respectively.
These regions belong to the \todo{SSU, ribozome, ... which is responsible for ...,}
and are thus present in virtually all \taxonname{bacteria} and \taxonname{eukaryotes}.
Often, prior to sequencing, these regions are amplified using polymerase chain reaction (PCR)
in order to produce more copies of them \todo{and thus help with error correction? citation}.
The resulting reads are then de-replicated again, which results in sequences called \emph{amplicons}. \todo{citation?}
...
% that is, single cellular strains cannot be resolved
%
