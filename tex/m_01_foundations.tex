...
\paperbox{
    This chapter mostly contains original contributions by Lucas Czech that were created for this thesis.
    Some of the text and figures introducing phylogenetic placements are however derived from
    the introductory sections of the following peer-reviewed open-access publications:
}{\paperart \paperpppp}{
%     All text relevant for this chapter was written by Lucas Czech.
%     All figures in this chapter were created by Lucas Czech.
%     All text was written and figures were created by Lucas Czech, unless explicitly stated otherwise.
    The respective texts and figures were originally created by Lucas Czech,
    and modified here to fit into the context of this chapter.
    Furthermore, some of the introductory figures in this chapter are derived from permissively licensed work of others,
    as stated below the respective figures.
%     We reused some of the introductory text and figures from the above publications.
}
...
% both of which is important for the role of \ac{DNA} for information storage.
...
Reads are typically stored in the \fileformat{fastq} file format \cite{Cock2009}.
...
the characters \nucleobase{A}, \nucleobase{C}, \nucleobase{G}, and \nucleobase{T}.
The resulting set of sequences is stored in formats such as the \fileformat{fasta} file format \cite{Pearson1988}.
...
% These studies yield a large set of short anonymous \ac{DNA} reads for each sample.
% The \ac{DNA} reads obtained from sequencing a sample represent all organisms present in the sample;
...
% \todo{add stuff about exponential stuff with PCA, how that affects abundances, see also normalization}
% \todo{also, talk about copy number bias etc}
...
such as \toolname{PaPaRa} \cite{Berger2011a,Berger2012} and
\toolname{hmmalign}, which is a subprogram of the \toolname{HMMER} suite \cite{Eddy1998,Eddy2009}.
...
see \citeay{Day1992a} for a review and comparison.
...
(\secref{ch:Foundations:sec:SequenceAnalysis:sub:Metagenomics}),
as it is unclear to which organisms they originally belonged to \cite{Oulas2015}.
Furthermore, for most microbial organisms,
% the traditional biological species concepts cannot be readily applied \cite{Blaxter2005}.
traditional systems of biological classification (\secref{ch:Foundations:sec:TreeOfLife:sub:TaxonomyNomenclature})
cannot be readily applied \cite{Blaxter2005}.
% In many studies, it is thus common practice to group closely releated individuals into so-called
In many studies, a pragmatic solution is thus to group closely releated individuals into so-called
\emph{operational taxonomic units} (OTUs) \cite{Sokal1963} instead. \acused{OTU}
...
see \citeay{Chen2013} for a review.
Common tools for \ac{OTU} clustering are \toolname{UCLUST} \cite{Edgar2010},
\toolname{VSEARCH} \cite{Rognes2016}, and \toolname{swarm} \cite{Mahe2014,Mahe2015}.
\toolname{swarm} works differently from other methods and produces more ``natural'' OTU clusters
...
that lists the abundances (counts) of each \ac{OTU} in each sample.
% it is commonly represented as an OTU table that lists the abundances of each OTU for each sample.
...
\secref{ch:Foundations:sec:PhylogeneticPlacement:sub:PlacementProcessing:par:Normalization}.
...
% \todo{maybe mention tree inferred from otus somewhere later?}
...
The \taxonname{Eukaryota} are further separated into kingdoms,
...
such as species, strains, individual organisms, or even single cells. %of a multicellular organism.
...
In rooted trees, these neighbors are the parent and the two children of the node, hence the name.
...
%     \mbox{RF}(T_1, T_2) ~=~ | B_1 \cup B_2 | - | B_1 \cap B_2 | ~=~ |\, ( B_1 \setminus B_2 ) \cup ( B_1 \setminus B_2 ) \,|
% \end{equation}
\begin{align}
    \label{ch:Foundations:sec:TreeOfLife:eq:RF-Distance}
%     \mbox{RF}(T_1, T_2) = \sfrac{1}{2} \,\cdot\, (\, | B_1 \cup B_2 | - | B_1 \cap B_2 | \,)
    ...
    &= |\, ( B_1 \setminus B_2 ) \cup ( B_2 \setminus B_1 ) \,|
\end{align}
...
A set of taxa is called \emph{monophyletic},
if it includes the most recent common ancestor of the taxa in the set, and includes all descendants of that ancestor.
In this work, we use an equivalent definition that also works for unrooted trees:
A set of taxa is called \emph{monophyletic},
if there is a bipartition (i.\,e., an edge) of the tree that separates these taxa from all other taxa of the tree.
In a rooted tree, the node at the end of that edge is then the common ancestor of these taxa.
A monophyletic set of taxa is also called a \emph{clade} of the tree;
in other words, a clade is a subtree that is separated from the rest of the tree by one edge.
For example, the set of taxa containing {\sffamily A}, {\sffamily B}, and {\sffamily C},
as well as the two small white inner nodes in \figref{fig:tree_vis_options:sub:unrooted} is monophyletic---%
these taxa form a clade of the tree.
Lastly, a non-monophyletic set of taxa is either called \emph{paraphyletic} or \emph{polyphyletic},
depending on whether the most recent common ancestor of the taxa is part of the set or not.
For instance, the taxa {\sffamily A} and {\sffamily C} in \figref{fig:tree_vis_options:sub:unrooted} are polyphyletic---%
they are not separated from the other taxa by a single edge, and do not contain their most recent common ancestor
(which is the white node next to {\sffamily C}).
...
% it is the most commonly used file format for phylogenetic trees.
...
\toolname{FastTree} \cite{Price2010}, \toolname{GARLI} \cite{Zwickl2006}, and \toolname{RAxML} \cite{Stamatakis2014,Kozlov2019a};
see \citeay{Zhou2018} for a critical review.
Bayesian inference on the other hand can for example be conduced using software tools such as
\toolname{BEAST} \cite{Suchard2018}, \toolname{MrBayes} \cite{Ronquist2003}, and \toolname{ExaBayes} \cite{Aberer2014}.
...
\begin{enumerate}
    \item Optimizing the tree topology $T$ (as well as the few branch lengths that are most affected by the respective topological change),
...
As mentioned before, most models of DNA evolution assume a \emph{time-reversible} Mar\-kov process,
...
For a more in-depth description, see \citeay{Yang2014}.
...
% which is for example present in the \fileformat{fastq} file format \cite{Cock2009}.
...
but there also exist studies that use different, or even a set of, marker genes \cite{Sunagawa2013a}.
Furthermore, other types of sequences such as \textsubscript{mi}tags \cite{Logares2014} can be used.
...
we proposed an automated approach for assembling suitable sets of reference sequences \cite{Czech2018-phat},
...
yielding a set of likelihood scores $\mathcal{L}(i)$ for each possible placement location of the \ac{QS}.
...
The probability of placing a \ac{QS} on a branch $q$ is called the \emph{\acl{LWR}}\acused{LWR}
(\acs{LWR}) %\footnote{In physics, \emph{weight} is the gravitational force acting on a \emph{mass}.
% It would hence be more precise to call this the \emph{likelihood mass ratio},
% as there is no gravity involved in phylogenetic placement.
% We however stick to established terminology here, and use weight and mass interchangeably.}
...
Conventional methods for this task, such as \toolname{BLAST} \cite{Altschul1990},
...
Furthermore, \toolname{BLAST} might yield suboptimal results \cite{Shah2018},
and the best \toolname{BLAST} hit does often \emph{not} represent the most closely related species \cite{Koski2001}.
...
which can be conducted using phylogenetic placement, as outlined in \chpref{ch:ConclusionFutureDirections}.
...
The recently proposed tools \toolname{RAPPAS} \cite{Linard2019} and \toolname{APPLES} \cite{Balaban2018}
...
We here briefly outline common types of problems due to normalization that also affect phylogenetic placement data.
...
by constructing \acfp{OTU}, as introduced in \secref{ch:Foundations:sec:SequenceAnalysis:sub:OTUs}.
...
For a thorough comparison of UniFrac to methods based on phylogenetic placement, see \citeay{Matsen2011a}.
...
as shown in \figref{fig:mass_ops:sub:masses}.
...
A larger example for the full \ac{RT} is shown in \figref{fig:masses_imbalances:sub:ReferenceTree}.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.8\linewidth]{mass_ops.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
        \label{fig:mass_ops:sub:masses}
    \end{subfigure}
    \begin{subfigure}{0pt}
        \phantomcaption
        \label{fig:mass_ops:sub:binning}
    \end{subfigure}
    \begin{subfigure}{0pt}
        \phantomcaption
        \label{fig:mass_ops:sub:squashing}
    \end{subfigure}
    \begin{subfigure}{0pt}
        \phantomcaption
        \label{fig:mass_ops:sub:imbalance}
    \end{subfigure}
    \caption[Operations on placement masses]{
        \textbf{Operations on placement masses.}
        \subref{fig:mass_ops:sub:masses}
        The \emph{edge mass} or \emph{mass of an edge} is the sum of \acsp{LWR} on that edge for all \acsp{QS} in a sample.
        \subref{fig:mass_ops:sub:binning}
        In order to reduce time and memory of the computations,
        masses can be \emph{binned} by summarizing them across \acp{QS} in intervals along the edges.
        \subref{fig:mass_ops:sub:squashing}
        The masses on corresponding edges of the \ac{RT} of two or more samples can be \emph{squashed} to represent
        the average mass distribution of the samples.
        For simplicity, we here use equal weights, and show edge masses instead of individual \acp{LWR}.
        \subref{fig:mass_ops:sub:imbalance}
        The \emph{edge imbalance} of an edge is the sum of masses on all edges on the root side of the edge
        ({\sffamily A+B}, with the root in subtree {\sffamily A} denoted as a gray dot)
        minus the sum of the masses on the edges on the non-root side ({\sffamily C+D}),
...
This approach is analogous to using proportional values for methods based on OTU abundance tables
(see \secref{ch:Foundations:sec:SequenceAnalysis:sub:OTUs}).
...
The masses on each edge of the tree can be quantized into $b$ discrete bins, as shown in \figref{fig:mass_ops:sub:binning}.
...
As shown later in \tabref{tab:hmp_binning_error},
...
by annotating the \ac{RT} with their (weighted) average per-edge mass distribution, as shown in \figref{fig:mass_ops:sub:squashing}.
...
and which also shows a larger example of squashing in \figref{fig:squashing}.
...
while ignoring the mass of the edge itself \cite{Matsen2011a}, as shown in \figref{fig:mass_ops:sub:imbalance}.
...
% An example of the imbalance calculation is shown in \figref{fig:masses_imbalances:sub:ReferenceTree}.
...
which we present in \chpref{ch:Balances}.
...
% % Hence, the previously mentioned caveats apply.
...
% % Edge imbalances use phylogenetic placement, and thus have a fixed reference tree that can be re-used.
...
An example of the edge masses and edge imbalances for a sample is shown in \figref{fig:masses_imbalances:sub:ReferenceTree}.
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.65\linewidth]{masses_imbalances.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
    ...
    \caption[Edge masses and imbalances]{
        ...
        \subref{fig:masses_imbalances:sub:ReferenceTree}
...
%         Because terminal edges only have a root side, their imbalance is not informative.
        \subref{fig:masses_imbalances:sub:Matrices}
        ...
        (\secref{ch:Foundations:sec:PhylogeneticPlacement:sub:ExistingMethods:par:EdgePCA}) or correlation coefficients.
    }
...
The three matrices are shown in \figref{fig:masses_imbalances:sub:Matrices}.
...
have been thoroughly examined by \citea{Evans2012}. %\citeauthor{Evans2012} \cite{Evans2012}.
...
Taking this information into account however improves the capacity for visualization and interpretation of the results.
...
% Edge PCA \cite{Matsen2011a} is a dimensionality reduction method for phylogenetic placement data,
...
the principal components can be visualized in form of a scatter plot of samples along the principal component axes.
...
these visualizations on the tree explain \emph{why} they are separated.
...
% \emph{Squash Clustering} \cite{Matsen2011a} is a hierarchical clustering method for phylogenetic placement data.
...
\begin{figure}[!pb]
    ...
    \caption[Squashing of edge masses]{
        ...
        The figure is based on the similar Figure~3/2 of \citeay{Matsen2011a}; see there for more details on squashing.
    }
    \label{fig:squashing}
\end{figure}
...
(\emph{squashes}, see \secref{ch:Foundations:sec:PhylogeneticPlacement:sub:PlacementProcessing:par:BinningSquashing})
clusters of similar samples by calculating their weighted average per-edge placement mass,
as shown in \figref{fig:squashing}.
...
% until only one large cluster remains that combines all samples.
...
% An exemplary clustering tree is shown later in \figref{fig:squash_edgepca:sub:squash}.
...
we show a similar visualization later in \figref{fig:centroids}.
