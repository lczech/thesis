...
We then applied the \texttt{chunkify} procedure as explained in \citeay{Czech2018-phat}
...
Lastly, given the per-chunk placement files produced by \toolname{EPA-ng}, we executed the \texttt{unchunkify} procedure of \citeay{Czech2018-phat}
...
For aligning, we used a custom \texttt{\acs{MPI}} wrapper of \toolname{PaPaRa~2.0} \citep{Berger2011a,Berger2012},
...
For our comparison of Placement-Factorization to the original Phylofactorization \cite{Washburne2017a},
we furthermore conducted OTU clustering of the sequences, using two different methods:
...
we used the \toolname{assign} command of our toll \toolname{gappa}.
...
Each set of OTUs was subsequently aligned with \toolname{MAFFT v7.310} \cite{Katoh2002,Katoh2013},
...
These two OTU trees were then used with the meta-data for conducting an analysis with \toolname{Phylofactor},
...
We then used the unconstrained \taxonname{Bacteria} tree of our \ac{PhAT} method \cite{Czech2018-phat} for phylogenetic placement.
