...
and a distance measure between them (see \secref{ch:Foundations:sec:PhylogeneticPlacement:sub:Distances}),
...
The methods take as input a set of $n$ samples, each consisting of a set of \acfp{QS} placed on a fixed \acf{RT}.
...
We used the Elbow method \cite{Thorndike1953},
...
We here evaluate our Phylogenetic $k$-means clustering (which uses edge masses and KR distances) and
...
see \figref{fig:cluster_kmeans_trees}.
...
%     \begin{subfigure}{0pt}
%         \phantomcaption
%         \label{fig:cluster_kmeans_trees:sub:mds}
%     \end{subfigure}
%     \begin{subfigure}{0pt}
%         \phantomcaption
%         \label{fig:cluster_kmeans_trees:sub:pca}
%     \end{subfigure}
%     \begin{subfigure}{0pt}
%         \phantomcaption
%         \label{fig:cluster_kmeans_trees:sub:edgepca}
%     \end{subfigure}
    \caption[Comparison of $k$-means clustering to Squash Clustering]{
        ...
        to the \ac{BV} dataset \cite{Srinivasan2012} in order to compare them to existing methods.
...
This can however be seen in \figref{fig:kmeans_all:sub:mds_em},
...
A similar visualization of the pairwise KR distances is shown in \figref{fig:kmeans_all:sub:pca_em}.
...
Lastly, \figref{fig:kmeans_all:sub:epca_ei} compares Imbalance $k$-means to Edge PCA.
...
%         \subref{fig:cluster_kmeans:sub:pca} and \subref{fig:cluster_kmeans:sub:edgepca} of the main text, respectively.
        ...
        of this cluster as shown in \figref{fig:cluster_kmeans_trees:sub:imbalance_tree} of the main text.
        \\
%       From the other Figure
         % Sub (C)
%         \subref{fig:cluster_kmeans_trees:sub:mds}
        Multidimensional scaling using the pairwise KR distance matrix of the samples,
        and colored by \emph{PKM}.
        % Sub (D)
%         \subref{fig:cluster_kmeans_trees:sub:pca}
        ...
        % Sub (E)
%         \subref{fig:cluster_kmeans_trees:sub:edgepca}
...
\begin{figure}[hpbt]
    ...
    \caption[Example of $k$-means cluster centroids visualization]{
        ...
        The cluster assignments are the same as in \figref{fig:cluster_kmeans_trees} of the main text;
...
%         Candidate points for such elbows are marked in orange here.
        ...
        These values are consistent with previous findings, for instance, \figref{fig:cluster_kmeans_trees} \todo{}:
...
\begin{figure}[hpbt]
    ...
    \caption[Clustering using Phylogenetic $k$-means on the HMP dataset]{
        ...
        See \figref{fig:cluster_kmeans_trees} \todo{} for the cluster assignment where $k$ is set to the original number of labels;
