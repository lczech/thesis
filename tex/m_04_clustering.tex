...
Furthermore, \subref{fig:kmeans_all:sub:mds_ei} and \subref{fig:kmeans_all:sub:pca_ei} show the MDS and the PCA plot,
...
The \acf{HMP} dataset (see \appref{supp:sec:DetailsEmpiricalDatasets:sub:HMP} for details)
...
\begin{figure}[hpbt]
    ...
    \caption[$k$-means cluster assignments of the \acs{HMP} dataset with $k:=8$]{
        ...
        See \figref{fig:hmp_kmeans_all_18} for the list of how the original body site labels were aggregated.
...
whereas those sites are spread across four of five clusters in \figref{fig:hmp_kmeans_all_18:sub:ei_unconstr}.
...
are mostly separated from each other into two blocks in both variants of $k$-means in \figref{fig:hmp_kmeans_all_18}.
...
Lastly, we also evaluated the clustering using the two \taxonname{General} trees of our \ac{PhAT} method (data not shown),
