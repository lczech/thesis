...
are provided in S1 Text. %Supplementary Section \nameref{supp:sec:DetailsEmpiricalDatasets}.
...
Secondly, for the \ac{TO} and \ac{HMP} datasets, we used our Automatic Reference Tree (ART) method \cite{Czech2018}
...
This small dataset with a total of \num{426 612} query sequences, thereof \num{15 060} unique,
was already analyzed with phylogenetic placement methods in the original publication.
We used it as an example of a well-designed study to asses our results using an \ac{ART} as reference tree.
In addition to the \taxonname{Bacteria} \ac{ART},
...
We placed the sequences on the unconstrained \taxonname{Eukaryota} reference tree obtained via our ART method \cite{Czech2018},
...
We then used the unconstrained \taxonname{Bacteria} tree of our ART method \cite{Czech2018} for phylogenetic placement.
...
\begin{itemize}
    \item \texttt{art}: Automatic Reference Tree method.
