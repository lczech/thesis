...
% \begin{equation*}
%     \label{ch:MaterialsMethods:sec:Balances:eq:Closure}
...
\eqnref{ch:MaterialsMethods:sec:Balances:eq:Balance}
represents the original ILR transform without a weighting scheme \cite{Egozcue2003},
\eqnref{ch:MaterialsMethods:sec:Balances:eq:WeightSums} equals the number of edges in $R$ and $S$, respectively,
and \eqnref{ch:MaterialsMethods:sec:Balances:eq:GeometricMean} is the standard (unweighted) geometric mean.
...
the conceptual similarity with the previously described \nameref{sec:Introduction:sub:Imbalances} becomes apparent:
...
We show an example of this computation in \figref{fig:balances}.
...
consequently, we use columns instead of rows of the edge masses matrix of \figref{fig:masses_imbalances:sub:Matrices},
...
As a first test of our adaptation of balances to placement data, we apply it to the \ac{BV} dataset.
...
and which corresponds to the imbalance matrix used for Edge PCA, c.\,f. \figref{fig:masses_imbalances:sub:Matrices}.
...
The first two components are shown in \figref{fig:bv_place_edge_balances_pca_scatter},
...
\begin{figure}[!htb]
    ...
    \caption{
        ...
        \subref{fig:bv_place_edge_balances_pca_scatter:sub:with_taxon_weighting}
        does not fully distinguish between the healthy (blue) and diseased (red) samples.
        For some reason, the component only takes \taxonname{Lactobacillus iners} into account,
        ...
        \\
        Subfigure \subref{fig:bv_place_edge_balances_pca_scatter:sub:without_taxon_weighting}
...
%         In fact, the only difference to Edge PCA is the use of balances instead of imbalances for the PCA.
        ...
        in \figref{fig:bv_place_edge_balances_pca_trees},
...
c.\,f., \figref{fig:epca}.
We show the results for PCA of the balances in \figref{fig:bv_place_edge_balances_pca_trees}.
...
\begin{figure}[!htb]
    ...
    \caption{
        ...
        in \figref{fig:bv_place_edge_balances_pca_scatter} can be interpreted:
...
Furthermore, as mentioned in the methods description,
...
We exemplify the correlation of the Nugent score with balances in \figref{fig:bv_place_edge_balances_correlation}.
...
With taxon weighting in \subref{fig:bv_place_edge_balances_correlation:sub:with_taxon_weighting},
the result is similar to the correlation with imbalances shown in \figref{fig:all_nugent:sub:srcc_ei}:
...
in \subref{fig:bv_place_edge_balances_correlation:sub:without_taxon_weighting}.
...
\label{ch:Balances:sec:ConclusionOutlook}
