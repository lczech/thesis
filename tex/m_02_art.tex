...
%shown in Equation \ref{eq:entropy}.
...
as shown in Algorithm \ref{algo:taxonomy_expansion}.
...
see \figref{fig:bv_comparison} for details.
...
and improves the accuracy of the first placement level, as shown in Section \ref{sec:Results:sub:MultilevelPlacement}.
...
An overview of the tree sizes is shown in Supplementary Table~\ref{tab:TaxonomicComposition}.
% Moved to supplement:
...
in Supplementary Section~\ref{sec:DetailsEvaluationReferenceTrees},
which also discusses differences between the constrained and unconstrained trees.
Details of the trees are shown in Supplementary Table~\ref{tab:ReferenceTreesOverview};
...
% In Figure~\ref{fig:detail_majorities_backbone},
% \nicetohave{
...
Further details are provided in Supplementary Table~\ref{tab:ReferenceTreesOverview}.
...
This is corroborated by the short average distances reported in Supplementary Table~\ref{tab:ReferenceTreesOverview}.
...
instead of using consensus sequences; see \figref{fig:single_seqs}.
...
(see \ref{ch:EmpiricalDatasets} and
...
are shown in Table~\ref{tab:TaxonomicComposition}.
...
as shown in Table~\ref{tab:TreeTopologyTests}.
The tests show that in all cases, the unconstrained trees fit the data significantly better.

Given these eight \acp{ART}, the evaluation was conducted as explained in the main text.
...
That is, we placed the \toolname{Silva} sequences of the five clades onto their respective \ac{ART}
...
The \acf{ART} method provides a means for automatically obtaining suitable reference trees
...
% even very broad \acp{ART} achieve high accuracy, particularly when using high-level clade constraints.
...
which is freely available under GPLv3 at \url{http://github.com/lczech/gappa}
(see \ref{ch:PipelineImplementation} for an overview of the corresponding commands).
...
% When using an \ac{ART} as backbone, clades are grouped based on their entropy, that is, how different they are.
...
see \ref{ch:EmpiricalDatasets} and \figref{fig:hmp_mds_epca} for details.
...
% \section{Evaluation of the Automatic Reference Trees}
...
\begin{figure}[hpbt]
    ...
    \caption[Accuracy comparison between unconstrained and constrained trees]{
        ...
        Figure~\ref*{fig:detail_majorities_backbone} of the main text and included here for ease of comparison.
        ...
        See Supplementary Section~\ref{sec:DetailsEvaluationReferenceTrees} and Table~\ref{tab:TreeTopologyTests}
...
%         and thus are excluded here for simplicity.
        For comparison, we also included Figure~\ref*{fig:detail_majorities_backbone:sub:num_br} of the main text again,
...
%         Furthermore, we used random initial starting trees for the tree inference.
        ...
        This is similar to our findings depicted in in Figure~\ref{fig:constraints_backbone}.
...
\begin{figure}[hpbt]
    ...
    \caption[Effect of using actual sequences (instead of consensus sequences) on placement accuracy]{
        ...
        see e.g., Figures~\ref{fig:constraints_backbone} and \ref{fig:consensi_backbone}.
        ...
        that were already evaluated in Figure~\ref{fig:consensi_backbone:sub:90_threshold}.
...
\begin{figure}[hpbt]
    ...
    \caption[Assessment of an Automatic Reference Tree for conducting Squash Clustering and Edge PCA]{
        ...
        Here, we test the behavior of the unconstrained \taxonname{Bacteria} tree obtained via our \ac{ART} method
        ...
        For details on the processing, see \ref{ch:EmpiricalDatasets}.
...
%         \subref{fig:bv_comparison:sub:edgepca_art} our \ac{ART} and
...
%         \\
%         This limitation can be overcome by either resolving the ART down to species level,
...
\begin{figure}[hpbt]
    ...
    \caption[Assessment of an Automatic Reference Tree for large dataset analyses]{
        ...
        Here, we test the unconstrained \taxonname{Bacteria} tree generated by our Automatic Reference Tree method
        ...
        See Table~\ref{tab:hmp_data_overview} for the mapping between the original labels and the ones used here.
...
%         See Figure~\ref{fig:bv_comparison} for a short explanation of this method.
        Similar to Figure~\ref{fig:bv_comparison:sub:edgepca_orig}, the plot forms a triangle of samples,
...
\begin{figure}[hpbt]
    ...
    \caption[Accuracy of the Automatic Reference Trees of five bacterial sub-clades]{
        ...
        These five clades are also highlighted in Figure~\ref{fig:clades};
...
%         The figure follows Supplementary Figure~\ref{fig:constraints_backbone} in layout.
        \\
        The placement accuracy on these sub-clades is slightly worse compared to the broad \taxonname{Bacteria} tree,
        which can be seen by comparison to Figure~\ref{fig:constraints_backbone}.
...
%         There are however also differences between the clades.
        ...
        In Figure \ref{fig:clades}, which shows the unconstrained \taxonname{Bacteria} tree,
