...
\todo{pipeline figures: phat, chunkify, etc. see gappa}

\todo{make sure, chunkify and unchunkify are consistenly used, same font, same reference, etc}

\todo{rename chapter to something more catchy!}
...
The mapping is based on the \textit{loose mapping} procedure by \citeay{Balvociute2017}.
...
We used the \toolname{CAMI} evaluation tool for taxonomic profilers \toolname{OPAL} \cite{Sczyrba2017}
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=0.94\linewidth]{cami_performance.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
...
% \afterpage{\clearpage}
\todo{the order of the table and figures is messed up, and the figures of the following section interfere... :-(}
...
First, using the \toolname{Silva} sequences and sub-taxonomies of these five clades,
...
\todo{maybe add a table similar to \tabref{tab:ReferenceTreesOverview}, listing sizes and accuracy etc.}
...
\todo{ref to implementation section in the supplement!}
...
\todo{implementation note (alexis: no) and ref to supplement (alexis: yes)!}
