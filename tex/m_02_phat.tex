...
which the underlying databases has classified into a clade of the taxonomy,
...
which we assess later in \secref{ch:AutomaticTrees:sec:Evaluation:sub:Accuracy:par:FurtherAspectsAndObservations}.
...
For example, the most significant change is observed for the \taxonname{Eukaryota} tree,
...
\todo{weird transition and abrupt ending. more flesh!}
...
%         \\
        ...
        across taxonomic ranks using the error metrics of \toolname{CAMI}:
...
Despite this, we find that the performance of our approach is in the mid-range of the tools evaluated by \toolname{CAMI}.
...
First, using the sequences and sub-taxonomies in \toolname{Silva} of these five clades,
...
\begin{figure}[hpbt]
    ...
    \caption[Accuracy of the \acsp{PhAT} of five bacterial sub-clades]{
        ...
        using the accuracy measurement as before (see \secref{ch:AutomaticTrees:sec:Evaluation:sub:Accuracy:par:MeasurementMethod}).
...
As mentioned in \secref{ch:AutomaticTrees:sec:Methods:sub:MultilevelPlacement} before,
...
\todo{implementation note and ref to supplement?}
