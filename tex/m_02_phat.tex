...
\paperbox{
    ...
    The text, figures, and tables in this chapter were created by Lucas Czech, with the following exception:
%     , unless explicitly stated otherwise.
    Pierre Barbera conducted the processing and analysis of the CAMI Challenge data
    as described in \secref{ch:AutomaticTrees:sec:Evaluation:sub:TaxonomicAssignmentProfiling},
    and helped to write the part of the above publication on which that section is based.
}
...
potentially including some outgroups from more distantly related species to also catch outliers.
...
(see \secref{ch:Foundations:sec:PhylogeneticPlacement:sub:UseCasesApplications:par:Comparison}).
...
% Third, we describe a preprocessing pipeline that allows for handling huge sequence datasets.
...
In this chapter, we introduce methods to overcome the aforementioned limitations, that is, to
...
the visualization of placements on such a large \ac{RT} might be confusing and thus hard to interpret.
...
which we call the \emph{Phylogenetic Automatic (Reference) Tree} (PhAT) method. \acused{PhAT}
The workflow of the method is summarized in \figref{fig:workflow_phat}.
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=\linewidth]{workflow_phat.pdf}
...
Firstly, the approach requires a measure to assess the similarity of a set of sequences to each other,
that is, their ensemble similarity.
...
Instead, we here use entropy \cite{Shannon1951} to define a measure
...
Some examples for the entropy when using four nucleobases are shown in \figref{fig:entropy}.
...
of the sequence data\-base to identify potential candidate groups of sequences
...
For instance, \figref{fig:tax_entropy} shows six sequences (S1--S6),
which the underlying database has classified into the \taxonname{Calyptosporidae} clade of the taxonomy,
...
we assess this problem later in \secref{ch:AutomaticTrees:sec:Evaluation:sub:Accuracy:par:FurtherAspectsAndObservations}.
...
\begin{figure}[t!hpb]
    \centering
    \includegraphics[width=0.8\linewidth]{tax_entropy.pdf}
...
Examples of the per-site entropies are shown in \figref{fig:tax_entropy}.
...
In our tests, the average deviation was \num{0.2}\%, see \tabref{tab:TaxonomicComposition}.
...
An exemplary majority rule consensus sequence is shown in \figref{fig:tax_entropy} below the six sequences S1--S6.
...
we can use them to infer a maximum likelihood tree, which is the resulting \emph{Phylogenetic Automatic (Reference) Tree} (PhAT).
\acused{PhAT}
...
for example as implemented in \toolname{SAT\'{e}} \cite{Liu2009,Liu2012}.
...
in \toolname{SEPP} \cite{Mirarab2012} and \toolname{TIPP} \cite{Nguyen2014},
...
a broad \taxonname{Eukaryota} \ac{BT} can be used for the first level,
...
\begin{figure}[pbt]
    \centering
    \includegraphics[width=0.75\linewidth]{multilevel_placement.pdf}
...
The workflow is also shown in \figref{fig:workflow_multilevel}.
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=0.85\linewidth]{workflow_multilevel.pdf}
    \caption[Multilevel pipeline for phylogenetic placement]{
        ...
        See \figref{fig:pipeline_placement} for a description of the typical placement pipeline.
    }
...
% in order to work with \acp{QS} which were not covered well in reference databases \cite{Mahe2017}.
...
at the second level.
% This approach is currently being tested in the context of the \toolname{UniEuk} project \cite{Berney2017}.
...
See \figref{fig:multilevel_placement} and \figref{fig:clades} for examples of such an overlap.
...
For instance, they store the raw sequencing output in \fileformat{fasta} \cite{Pearson1988}
or \fileformat{fastq} \cite{Cock2009} format (see \secref{ch:Foundations:sec:SequenceAnalysis:sub:GenomeSequencing}).
...
one can pre-process the sequences as summarized in \figref{fig:workflow_chunkify} and explained in the following
(see \appref{ch:PipelineImplementation} for implementation details).
...
\begin{figure}[pbt]
    \centering
    \includegraphics[width=\linewidth]{workflow_chunkify.pdf}
    \caption[Pre-processing pipeline for phylogenetic placement]{
        ...
        See \figref{fig:pipeline_placement} for the typical placement pipeline.
%         First, the set of files containing the query sequences of each sample is ``chunkified'':
%         Instead of per-sample files, a set of sequence chunk files is produced,
%         which contain each unique sequence once, as well as a set of mapping files
%         that store the original abundances of the sequences per original sample file.
    }
...
as explained in \secref{ch:Foundations:sec:PhylogeneticPlacement:sub:PipelineAndComputation}.
% using programs such as  \toolname{PaPaRa} \cite{Berger2012,Berger2011a}
% or \toolname{hmmalign} \cite{Eddy1998,Eddy2009},
% for example by \toolname{pplacer}, \toolname{RAxML-EPA} or \toolname{EPA-ng} \cite{Matsen2010,Berger2011,Barbera2018}.
...
Furthermore, to demonstrate the scalability of our method, and for the evaluation of the methods presented in later chapters,
...
For an overview and further details on the datasets used in our evaluation, see \appref{ch:EmpiricalDatasets}.
% The improvements presented here were necessary in order to process and place these datasets in manageable time,
Without the improvements presented here, these datasets could not have been processed and placed in manageable time.
As we use the placement results of these datasets on the \acp{PhAT} in the evaluations of later chapters,
these improvements represent necessary preprocessing steps for this work,
and for large analyses using phylogenetic placement in general.
...
we used the ``SSU Ref NR 99'' sequences of the \toolname{Silva} database \cite{Quast2013} version 123.1
...
We use the \toolname{Silva} alignment as-is,
and thereby implicitly assume that it is of sufficient quality for our purposes;
see \secref{ch:AutomaticTrees:sec:Evaluation:sub:Accuracy:par:FurtherAspectsAndObservations}
...
were chosen to be large enough to cover the diversity well,
while still being computationally feasible and visually interpretable for the subsequent steps.
The target size for the \taxonname{General} tree was set to \num{2 000} taxa,
...
which represents the fully resolved \taxonname{Archaea} taxonomy at the \taxonname{Genus} level.
...
The \taxonname{Archaea}, being a small enough clade, did not have a target size, but were fully resolved.
}
...
(the \taxonname{General} sequences, as well as the three domain-specific sequences),
...
Unconstrained trees were inferred using \toolname{RAxML~8.2.8} \cite{Stamatakis2014}.
...
The relative Robinson-Foulds distances \cite{Robinson1981}
...
The tests were performed with \toolname{IQ-TREE v1.5.6} \cite{Nguyen2015a}
...
using \num{10 000} resamplings for the RELL method \cite{Kishino1990}.
The table shows that the unconstrained trees fit the sequence data significantly better
in all four cases and for all test statistics.
\\
Columns are as follows.
logL and deltaL: log likelihood score and difference in log likelihood scores between constrained and unconstrained tree.
bp: bootstrap proportion using RELL method \cite{Kishino1990}.
p-(W)KH: p-value of the one sided and the weighted Kishino-Hasegawa test \cite{Kishino1989}.
p-(W)SH: p-value of the (weighted) Shimodaira-Hasegawa test \cite{Shimodaira1999}.
c-ELW: Expected Likelihood Weight \cite{Strimmer2002}.
p-AU: p-value of approximately unbiased (AU) test \cite{Shimodaira2002}.
}
...
which is computed from corresponding species-level sequences in \toolname{Silva} that share the same taxonomic label.
...
In order to conduct this accuracy evaluation,
...
The placement accuracy results for the four unconstrained and the four constrained trees
are shown in \figref{fig:constraints_backbone}.
Further details of the trees and the respectively achieved placement accuracies
are provided in \tabref{tab:ReferenceTreesOverview}.

\begin{figure}[hpbt]
    \centering
    \includegraphics[width=\linewidth]{constraints_backbone.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
...
%     Weighted distances to expected edges for
    \caption[Induced placement accuracy on the \acsp{PhAT}]{
        ...
        and in \subref{fig:constraints_backbone:sub:branch_unconstr}, more than 95\% of the \taxonname{Eukaryota} (purple)
...
% \begin{figure}[hpbt]
%     \centering
%     \includegraphics[width=\linewidth]{detail_majorities_backbone.pdf}
...
Details of the four unconstrained (U) and the four constrained (C) trees are shown.
``Size'' is the number of leaves in the tree, that is, the number of consensus sequences that the tree was inferred from,
see \tabref{tab:TreeTopologyTests}.
``\% Seqs.'' is the percentage of sequences from \toolname{Silva} placed on it.
...
Average distances of the placed sequences to their respective expected branch are listed
...
The EDPL is the sum of the pairwise distances between the placements of a sequence weighted by their probability \cite{Matsen2010}.
}
...
This might be because the \taxonname{Bacteria} have the most sequences in \toolname{Silva}, that exhibit a high diversity.
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=\linewidth]{consensi_backbone.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
...
%         The evaluation method is described in the text.
        ...
        \subref{fig:consensi_backbone:sub:cavener} Cavener's method \cite{Cavener1987,Cavener1991a},
...
By using alternative consensus methods, the consensus sequences and thus the alignment sites change.
...
that is, when comparing, e.\,g., the \taxonname{Bacteria} trees obtained with different consensus methods to each other,
we observed an average relative Robinson-Foulds (RF) distance \cite{Robinson1981} of 49.5\%;
see \secref{ch:Foundations:sec:TreeOfLife:sub:PhylogeneticTrees:par:TreeProperties} for details on the RF distance.
...
does not change substantially compared to the accuracy obtained for the unconstrained trees shown in \figref{fig:consensi_backbone}.
...
On the other hand, the \taxonname{Bacteria}, 
for which there is by far the largest amount of sequences in \toolname{Silva}, perform worst.
...
\figref{fig:consensi_backbone:sub:50_threshold}--\subref{fig:consensi_backbone:sub:90_threshold}.
...
as described before.
The results for the four unconstrained trees are shown in \figref{fig:single_seqs}.

\begin{figure}[hpbt]
    \centering
    \includegraphics[width=\linewidth]{single_seqs.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
...
For example, the most pronounced change is observed for the \taxonname{Eukaryota} tree,
...
For example, \toolname{Sativa} \cite{Kozlov2016},
...
% However, errors are expected due to incongruence between the taxonomy and the phylogeny \cite{Moreira2000},
% as well as due to taxonomically mislabeled sequences \cite{Kozlov2016}.
...
The results reveal that the \ac{PhAT} reproduces certain aspects of the original study %results of previous studies
...
First, we conducted Squash Clustering \cite{Matsen2011a}
...
\begin{figure}[hpbt]
    ...
    \caption[Assessment of a \acs{PhAT} for conducting Squash Clustering]{
        ...
        \subref{fig:bv_squash:sub:squash_orig} the original reference tree of \citeay{Srinivasan2012}.
        Subfigure \subref{fig:bv_squash:sub:squash_orig} is a recalculation of Figure~1(A) of \citeay{Srinivasan2012},
...
While \figref{fig:bv_squash:sub:squash_orig} shows some small branch lengths and even a separated sub-clade
of samples with low Nugent score,
these branches have a length of virtually zero in \figref{fig:bv_squash:sub:squash_art}.
As shown in \citeay{Srinivasan2012}, the healthy patients are divided into two classes,
...
Second, we conducted an Edge PCA \cite{Matsen2011a}
...
\begin{figure}[hpbt]
    ...
    \caption[Assessment of a \acs{PhAT} for conducting Edge PCA]{
        ...
        \subref{fig:bv_edgepca:sub:edgepca_orig} the original reference tree of \citeay{Srinivasan2012}.
        Subfigure \subref{fig:bv_edgepca:sub:edgepca_orig} is a recalculation of Figure~3(A) of \citeay{Srinivasan2012}.
...
are not separated from each other in the Edge PCA plot of \figref{fig:bv_edgepca:sub:edgepca_art}.
Hence, the two classes of healthy patients do again not exhibit
the separation based on the two \taxonname{Lactobacillus} species that is apparent when using the original reference tree
as in \figref{fig:bv_edgepca:sub:edgepca_orig}.
...
for example, contains species sequences of the relevant \taxonname{Lactobacillus} clades.
...
For this, we used the Human Microbiome Project (HMP) \cite{Huttenhower2012,Methe2012} data,
...
\begin{figure}[thb!]
    \centering
    \includegraphics[width=\linewidth]{mds_epca.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
    ...
    \caption[Assessment of a \acs{PhAT} for large dataset analyses]{
        ...
        See \tabref{tab:hmp_data_overview} for the mapping between the original labels and the groups used here.
    }
    \label{fig:hmp_mds_epca}
\end{figure}

% The phylogenetic Kantorovich-Rubinstein (KR) distance \cite{Matsen2011a,Evans2012} between two samples,
...
% It is a generalization of the UniFrac distance \cite{Lozupone2005,Lozupone2007a}.
...
Even different oral regions are mostly separated, although there is quite some overlap.
...
Taxonomic profiling is the prediction of the taxonomic identities and their relative abundances of sequences
from samples obtained via shotgun sequencing (\secref{ch:Foundations:sec:SequenceAnalysis:sub:Metagenomics});
it does not result in taxonomic assignments for individual sequences (although this can be an intermediate step),
but yields a summary of the abundances of different taxa in the samples \cite{Meyer2019}.
...
To perform the evaluation, we conducted parts of the \toolname{CAMI} Challenge \cite{Sczyrba2017},
which is a community-driven effort to assess taxonomic profiling methods using a common set of benchmark datasets.
To assess the feasibility of using trees generated with \ac{PhAT} to obtain taxonomic profiles of microbiome data,
we utilized the \taxonname{mouse gut} dataset of the 2nd \toolname{CAMI} Challenge \cite{Bremges2018}.
See \appref{supp:sec:DetailsEmpiricalDatasets:sub:MouseGut} for details on our processing of this dataset.
...
on our unconstrained and constrained \taxonname{Bacteria} trees.
% The sequences of the 16S region make up \num{\approx 0.08}\% of the total dataset.
...
in analogy to the method used by \toolname{Sativa} \cite{Kozlov2016}.

Unfortunately, the \toolname{CAMI} Challenge requires taxonomic assignments
that conform with the \toolname{NCBI} taxonomy \cite{Sayers2009,Benson2009}.
As our reference tree is however based on the \toolname{Silva} taxonomy \cite{Yilmaz2014},
we thus had to compute a mapping between the two taxonomies. %, which introduces some incongruences \cite{Balvociute2017}.
...
The mapping is based on the \textit{loose mapping} procedure of \citeay{Balvociute2017}.
More specifically, we tried to map taxonomic paths to their name, rank, and ID in the \toolname{NCBI} taxonomy,
...
as the \toolname{Silva} and \toolname{NCBI} taxonomies are far from congruent \cite{Balvociute2017}.
...
(see also \appref{supp:sec:DetailsEmpiricalDatasets:sub:MouseGut}).
This means that our taxonomic profiling only uses a small fraction of the available data.

% from OPAL paper:
% The assessed profilers were CommonKmers (corresponding to MetaPalette 1.0.0) [2, 42], CAMIARKQuikr 1.0.0 [43], abbreviated Quikr (a combination of Quikr [8], ARK [9], and SEK [10]), TIPP 2.0.0 [12], Metaphlan 2.2.0 [5], MetaPhyler 1.25 [6], mOTU 1.1 [7], and FOCUS 0.31 adapted for CAMI [4]. 

The resulting per-read assignment was then used to generate a taxonomic profile of the data.
We used the \toolname{CAMI} evaluation tool for taxonomic profilers \toolname{OPAL} \cite{Sczyrba2017,Meyer2019}
to compare our approach to competing software on the ``gold standard'' result for the dataset.
\toolname{OPAL} assesses the performance of a set of tools relative to each other.
We compared our approach to the profilers that were tested in the original publications \cite{Sczyrba2017,Meyer2019},
which are:
\toolname{CommonKmers} (corresponding to \toolname{MetaPalette} 1.0.0) \cite{Koslicki2016,MetaPalettev100},
\toolname{CAMIARKQuikr} 1.0.0 \cite{CAMIARKQuikrv100} (which is a combination of
\toolname{Quikr} \cite{Koslicki2013}, \toolname{ARK} \cite{Koslicki2015}, and \toolname{SEK} \cite{Chatterjee2014},
and abbreviated here as \toolname{Quikr}),
\toolname{TIPP} 2.0.0 \cite{Nguyen2014},
\toolname{MetaPhlAn} 2.2.0 \cite{Segata2012},
\toolname{MetaPhyler} 1.25 \cite{Liu2011},
\toolname{mOTU} 1.1 \cite{Sunagawa2013a}, and
\toolname{FOCUS} 0.31 adapted for CAMI \cite{Silva2014}.

We here show the most important \toolname{OPAL} results:
\figref{fig:cami_performance} compares the tools based on different error metrics;
\tabref{tab:cami_rankings} shows the scores and ranks of our approach compared to the other \toolname{CAMI} participants.
% % \figref{fig:cami_alpha_diversity} finally shows how the tools compare with respect to their estimation of diversity,
% in comparison to the gold standard ground truth.
...
Details on the error metrics and their interpretation are explained in the \toolname{OPAL} publication \cite{Meyer2019}.
...
\caption[CAMI scores and ranks]{
    ...
    the 2nd \toolname{CAMI} challenge \cite{Sczyrba2017,Bremges2018}.
    Here, we compare the taxonomic assignment and profiling based on our \acp{PhAT}
    to the tools that took part in the 2nd \toolname{CAMI} challenge.
    For this, we used the unconstrained and constrained \taxonname{Bacteria} tree,
    ...
    \\
    Four metrics are used in \toolname{CAMI} for evaluating tools:
    ...
    The procedure of the scoring and ranking is explained in detail in the Online Supplement of \citeay{Sczyrba2017}.
    \\
    Despite the caveats and limitations that are explained in the text,
    using our \acp{PhAT} trees to obtain a taxonomic profile yields rankings in the middle of the field for all metrics,
    even when comparing them to tools dedicated to the purpose of taxonomic profiling.
}
\label{tab:cami_rankings}
{
    ...
    \begin{tabular}{lrrrrrrrrrr}
        ...
        \midrule
        \toolname{MetaPhlAn}           &   1814    &   1   &   958     &   3   &  75       &   1   &  731      &   2   & 50    &   1   \\
        \toolname{MetaPhyler}          &   2995    &   2   &   314     &   1   &  2119     &   7   &  322      &   1   & 240   &   5   \\
        \toolname{CommonKmers}         &   3333    &   3   &   1448    &   6   &  409      &   2   &  1318     &   7   & 158   &   3   \\
        \toolname{mOTU}                &   3751    &   4   &   1703    &   8   &  488      &   3   &  1260     &   5   & 300   &   6   \\
        \textbf{PhAT (C)}              &   3784    &   5   &   1202    &   4   &  1116     &   4   &  1280     &   6   & 186   &   4   \\
        \textbf{PhAT (U)}              &   3933    &   6   &   1436    &   5   &  1153     &   5   &  1208     &   4   & 136   &   2   \\
        \toolname{TIPP}                &   4263    &   7   &   892     &   2   &  2126     &   8   &  930      &   3   & 315   &   7   \\
        \toolname{FOCUS}               &   6153    &   8   &   2079    &   9   &  1636     &   6   &  2018     &   8   & 420   &   8   \\
        \toolname{Quikr}               &   6838    &   9   &   1488    &   7   &  2398     &   9   &  2453     &   9   & 499   &   9   \\
        \bottomrule
    \end{tabular}
    \end{center}
}
...
% Try to print all the above first, instead of concatenation
% with the figures following below in the next section...
% Doesn't seem to work right now :-(
% https://tex.stackexchange.com/a/88658
% \afterpage{\clearpage}
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=0.918\linewidth]{cami_performance.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
    ...
    \caption[CAMI profiling results]{
        ...
        The figure compares the taxonomic %assignment and
        profiling conducted with our \taxonname{Bacteria} trees
        to the tools of the 2nd \toolname{CAMI} challenge \cite{Sczyrba2017,Bremges2018},
        and was created with the \toolname{OPAL} tool \cite{Meyer2019}.
        The unconstrained and constrained tree are abbreviated here as ``PhAT (U)'' and ``PhAT (C)'', respectively.
%         \\
        Subfigure \subref{fig:cami_performance:sub:relative} shows the \emph{relative} performance of the tools
        across taxonomic ranks using the \toolname{CAMI} error metrics:
        Weighted Unifrac error, L1 norm error, recall (completeness), precision (purity) and false positives.
%         The error metrics were divided by their respective maximal value
%         to allow viewing them on the same scale
%         and to allow relative performance comparisons.
%         \\
        Subfigure \subref{fig:cami_performance:sub:absolute} shows the \emph{absolute}
        recall (completeness) and precision (purity) for each tool across the taxonomic ranks.
%         \\
...
%         with data from, and following the protocol of, the 2nd CAMI challenge \cite{Sczyrba2017,Bremges2018}.
%         Here, we compare the taxonomic assignment and profiling conducted with our trees
%         to the tools that took part in the 2nd CAMI challenge.
%         For this, we used the unconstrained and constrained \taxonname{Bacteria} trees,
...
% Despite the aforementioned caveats of taxonomic profiling using phylogenetic placements in the \toolname{CAMI} setting,
...
we find that the performance of our approach is in the mid-range of the tools evaluated by \toolname{CAMI}.
...
% Fucking LaTeX loses figures if we do not include this ugly clear page here...
% \clearpage
...
The same clades were already scrutinized in \toolname{Sativa} \cite{Kozlov2016}.
\figref{fig:clades} shows the unconstrained \taxonname{Bacteria} tree from the previous evaluations,
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=\linewidth]{clades.pdf}
    \vspace*{0.5em}
    \caption[Unconstrained \taxonname{Bacteria} tree with five bacterial sub-clades]{
        ...
        We call this the ``overlap'' with the \taxonname{Bacteria} tree.
    }
    \label{fig:clades}
\end{figure}
...
First, using the \toolname{Silva} sequences and sub-taxonomies of these five clades as input,
...
That is, we placed the \toolname{Silva} sequences of the five clades onto their respective \acp{PhAT}
...
\begin{figure}[hpbt]
    \centering
    \includegraphics[width=\linewidth]{multilevel.pdf}
    \begin{subfigure}{0pt}
        \phantomcaption
    ...
    \caption[Accuracy of the \acsp{PhAT} of five bacterial sub-clades]{
        ...
        which were already scrutinized in \citeay{Kozlov2016},
        ...
        (see \secref{ch:AutomaticTrees:sec:Evaluation:sub:Accuracy:par:MeasurementMethod}).
...
%         The figure follows \figref{fig:constraints_backbone} in layout.
        \\
        The top row (Subfigures \subref{fig:multilevel:sub:edge_unconstr} and \subref{fig:multilevel:sub:edge_constr})
        shows discrete distances in number of branches;
        the bottom row (Subfigures \subref{fig:multilevel:sub:branch_unconstr} and \subref{fig:multilevel:sub:branch_constr})
        shows continuous distances in branch length units.
        The left side shows the accuracy of the unconstrained trees,
        the right side shows the accuracy for trees constrained by the \toolname{Silva} taxonomy.
    }
...
% \begin{table}[htbp]
% \caption[Subclade sizes and accuracy]{
%     \textbf{Subclade sizes and accuracy.}
%     The table lists how many taxa each of the selected five subclades has
%     in the first level tree (our unconstrained \taxonname{Bacteria} tree, see \figref{fig:clades}) and
%     in the second level trees (used for the evaluation shown in \figref{fig:multilevel}).
% }
% \label{tab:subclade_sizes}
% {
%     \begin{center}
%     \begin{tabular}{lrr}
%         \toprule
%                  & \multicolumn{2}{c}{Tree Size} \\
%         Subclade & First Level & Second Level    \\
%         \midrule
%         × & × & ×\\
%         × & × & ×\\
%         × & × & ×
...
The placement accuracy is slightly worse for the sub-clade trees
than for the eight comprehensive \acp{PhAT} evaluated before
(see \secref{ch:AutomaticTrees:sec:Evaluation:sub:Accuracy:par:UnconstrainedConstrainedResults}),
...
is around \num{0.4}, which is indicated by the end of the red line in the lower two plots of \figref{fig:multilevel}.
On the other hand, the \taxonname{Firmicutes} generally have the lowest accuracy.
In \figref{fig:clades}, which shows the unconstrained \taxonname{Bacteria} tree,
the \taxonname{Firmicutes} clade exhibits many paraphyletic clades,
...
which might explain why the \taxonname{Firmicutes} score worst in \figref{fig:multilevel}.
...
For example, Table~2 of \citeay{Janda2007} lists \num{10} bacterial genera
...
(as introduced in \secref{ch:AutomaticTrees:sec:Methods:sub:MultilevelPlacement}).
...
Thus, for multilevel placement, they will be assigned to the correct second level \ac{CT}.
...
as only 94.7\% of the \taxonname{Firmicute} sequences are placed into the corresponding correct clade.
This can be explained by the high amount of paraphyly of this clade, as mentioned above,
which is a known issue \cite{Parks2018}. % found a fitting fresh preprint thanks to Alexey!
...
% We already used this in \cite{Mahe2017}.
...
As already mentioned in \secref{ch:AutomaticTrees:sec:Methods:sub:MultilevelPlacement},
...
We presented methods and algorithms to facilitate and accelerate
phylogenetic placement of large environmental sequencing studies. % sets of sequence samples.
The \acf{PhAT} method (\secref{ch:AutomaticTrees:sec:Methods:sub:PhAT}) provides a means
for automatically obtaining suitable reference trees by using the taxonomy of large sequence databases.
Using the \toolname{Silva} database as a test case,
we showed that it can be applied for accurately (pre-)placing environmental sequences into taxonomic clades.
In combination with our multilevel placement approach (\secref{ch:AutomaticTrees:sec:Evaluation:sub:MultilevelPlacement}),
...
in conjunction with phylogenetic placement (\secref{ch:AutomaticTrees:sec:Evaluation:sub:TaxonomicAssignmentProfiling}).
...
we recommend using \toolname{Sativa} \cite{Kozlov2016}
...
does not necessarily provide resolution at the \taxonname{species} level \cite{Dunthorn2014}.
...
As we show, our multilevel placement method (\secref{ch:AutomaticTrees:sec:Methods:sub:MultilevelPlacement})
as well as the preprocessing pipeline (\secref{ch:AutomaticTrees:sec:Methods:sub:DataPreprocessing})
...
we refer to our results presented in \citeay{Mahe2017}.

Apart from exploring sequence data from unknown environments,
