...
\paperbox{
    ...
    During the revision of the above publication, reviewer Michael Robeson suggested to compare our methods
    of Chapters \ref{ch:Visualization}--\ref{ch:Clustering} to Phylofactorization,
    and thereby inspired this and the previous chapter (Chapters \ref{ch:Balances}--\ref{ch:Factorization}).
    Furthermore, a lot of mathematical understanding and many design choices emerged from discussion with Alex Washburne.
}
...
see \citeay{Washburne2018}
% see \citeay{Washburne2017a} %and \citeay{Washburne2018}
...
For details on the usage of \acp{GLM} for Phylofactorization, see \citeay{Washburne2017a}.
...
see \citeay{Washburne2017a} for some examples.
...
this is suitable for identifying clades that can serve as a bio-indicator, i.\,e., in order to predict diseases.
...
For the sake of simplicity, we hence focus on linear (gaussian) modeling of $C_e \sim Z$,
...
the independent variables (e.\,g., the meta-data $Z$) are called the \emph{predictor variables} or \emph{explanatory variables},
while the dependent variable (e.\,g., the contrasts $C_e$) is called the \emph{response variable}.
...
The expected value of $Y$ is usually assumed to be the mean value $\mu$ of the distribution of $Y$\hspace{-0.2em},
meaning that $\mathbb{E}(Y|X) = \mu$.
...
As described above, the \ac{GLM} uses the link function to allow for non-linear behaviour of the response variable $Y$\hspace{-0.2em}.
However, as evident from \eqnref{ch:Factorization:sub:GLM:eq:eta},
the predictor variables $X$ are linearily combined to form $\eta$.
Note that this does not necessarily impose a linear relationship of $X$ with $Y$:
...
For given data $X$ and $Y$\hspace{-0.1em}, and a link function $g$, the model is fit to the data by estimating the parameters $\beta$.
...
% Similar to the vis chapter, we need a custom bar length for the Y to look good...
% https://tex.stackexchange.com/a/22134/171851
% \newcommand{\overbar}[1]{\mkern 1.5mu\overline{\mkern-1.5mu#1\mkern-1.5mu}\mkern 1.5mu}
% \newcommand{\barredy}{\mkern 1mu\overline{\mkern-1mu{}Y\mkern1mu}\mkern-1mu\;\!}
\newcommand{\barredy}{\mkern 1.5mu\overline{\mkern-1.5mu{}Y\mkern-1.5mu}\mkern 1.5mu}
...
which is optimized by simply predicting the mean $\beta_0 = \barredy$ for all values.
...
Thus, the deviance of a model is simply an expression of its likelihood:
...
Hence, in linear regression, the residuals of the null model are the differences from the mean $\barredy$\hspace{-0.3em},
and its deviance is equivalent to the total sum of squares of the data $Y$:

\begin{equation}
    \label{ch:Factorization:sub:GLM:eq:null_deviance}
    D_n ~=~ \sum_{i=1}^{n} \left( Y_i - \barredy \right)^2
\end{equation}
...
using the explanatory variables $X$, than just predicting the mean value $\barredy$.
...
%     \operatorname{VAR}(y) &= \frac{1}{n} \sum_{i=1}^{n} \left( y_i - \barredy_i \right)^2 \\
%     \operatorname{MSE}(y) &= \frac{1}{n} \sum_{i=1}^{n} \left( y_i - \hat{y}_i \right)^2
    \omega %~=~ D_n - D_m %\sigma^2(y) - \delta^2(y)
    ~=~ \sum_{i=1}^{n} \left( Y_i - \barredy \right)^2
    ~-~ \sum_{i=1}^{n} \left( Y_i - \hat{Y}_i \right)^2
%     \omega &= \operatorname{VAR}(y) - \operatorname{MSE}(y) \\ \\
%       &= \frac{1}{n} \sum_{i=1}^{n} \left( y_i - \barredy \right)^2
%        - \frac{1}{n} \sum_{i=1}^{n} \left( y_i - \hat{y}_i \right)^2
\end{equation}

with $\barredy$ being the mean, and $\hat{Y}_i$ being the predicted values.
...
By maximizing the objective function $\omega$ across all edges, we hence are looking for the edge
where the balances are most predictable from their meta-data. %when taking the meta-data into account,
...
such as its insensitivity to singular large values.
...
This can also be seen in our visualization of the values of the objective function,
as for example shown later in \figref{fig:pf_bv_place_tw_ovs}.
...
We analyzed the \acf{BV} dataset \cite{Srinivasan2012} with our Placement-Factori\-zation with and without taxon weighting,
...
Hence, we here focus on the results obtained with all three meta-data features taken into account.
...
That is, there are several clades or branches of the reference tree
where the placement mass differs between healthy and sick patients.
It is thus expected that a Phylofactorization of these data exhibits some variation in the clades being found,
...
%              &  Phylofactorization (VSEARCH)          & Phylofactorization (swarm)        & Placement-Factorization   \\
             &  Original (\toolname{VSEARCH})          & Original (\toolname{swarm})        & Placement-Factorization   \\
        \midrule
...
\begin{figure}[btp!]
    ...
    \caption[Objective function values of Placement-Factorization with taxon weighting of the \acs{BV} dataset]{
        ...
        The winning edge of each iteration is marked by a black arrow, and the resulting clade is marked with a black arc.
...
than expected when using taxon weighting; the winning edge is marked with an arrow.
...
as presented in \citeay{Washburne2019}, which we discuss later.
...
\begin{figure}[!htb]
    ...
    \caption[Ordination of the first two factors of the \acs{BV} dataset]{
        ...
        in Subfigure \subref{fig:pf_bv_place_ilr_ordination:sub:with_taxon_weighting} (with taxon weighting),
        and see \figref{fig:factors_tree} and \tabref{tab:bv_phylofactor_clades}
        for the edges corresponding to the axes in Subfigure \subref{fig:pf_bv_place_ilr_ordination:sub:without_taxon_weighting}
...
Note that such scatter plots can only reasonably visualize the first two or three factors,
...
We here show an analysis of a subset of the \acf{HMP} dataset \cite{Huttenhower2012,Methe2012},
...
The original publication of Phylofactorization used a dataset from \citeay{Caporaso2011} as one of their case studies,
which comprises oral and fecal samples from the human microbiome.
See Figure~4 and Supplementary Figures~S3--S8 of \citeay{Washburne2017a} for the original analysis with Phylofactorization.
...
In particular, the larger clades found by the variant with taxon weights
...
of the two large clades in the upper half of \figref{fig:multi_factors_tree},
...
The actual differences in taxa (such as the \taxonname{Proteobacteria} not being found by our variants)
...
\begin{figure}[!htbp]
    ...
    \caption[Ordination of an oral/fecal subset of the \acs{HMP} dataset]{
        ...
        This type of per-factor visualization is similar to, e.\,g., Figure~4 of \citeay{Washburne2019}.
...
Placement-Factoriza\-tion identifies branches of the reference tree, called \emph{phylogenetic factors},
...
in that it further allows to identify nested dependencies within sub-clades of the reference tree,
and that it can take multiple meta-data features into account at once,
however without the immediate visualization of the direction of the correlation/dependency with these features.
...
the taxon weighting scheme used in the balances computation, as explained in \secref{ch:Balances:sec:Methods:sub:EdgeWeights}.
